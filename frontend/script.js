...
if (!searchInput) {
  console.warn(
    "Search input not found. Make sure there is an <input> inside .searchbar."
  );
} else {
  // container that holds product cards
  const productsContainer =
    document.getElementById("productsContainer") || document;
  ...
  function filterProducts() {
    ...
    cards.forEach((card) => {
      ...
      if (q === "" || name.includes(q)) {
        ...
        if (!card._originalDisplay)
          card._originalDisplay = getDefaultDisplay(card);
